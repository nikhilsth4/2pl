...
def begin_transaction(transaction_id, transaction_table, global_timestamp):
    ...
    if transaction_id not in transaction_table:
        ...
        print(f"Transaction T{transaction_id} is created and starts at TS({global_timestamp}).")
    ...
    if item_id in lock_table:
        ...
        if current_lock["lock_state"] == "read":
            if current_lock["holding_transaction"] == transaction_id:
                ...
                if current_transaction["timestamp"] < current_lock["holding_transaction"]["timestamp"]:
                    current_transaction["transaction_state"] = "waiting"
                    current_lock["waiting_transactions"].append(transaction_id)
                    print(f"T{transaction_id} is younger and waits for {current_lock['holding_transaction']} to release {item_id}.")
                else:
                    current_transaction["transaction_state"] = "aborted"
                    print(f"T{transaction_id} is older and aborted due to conflict with T{current_lock['holding_transaction']} on {item_id}.")
        else:
            if current_transaction["timestamp"] < current_lock["holding_transaction"]["timestamp"]:
                current_transaction["transaction_state"] = "waiting"
                current_lock["waiting_transactions"].append(transaction_id)
...
def parse_operation(line):
    ...
    op = parts[0]
    if len(parts) > 1:
        tid = int(parts[0][1:])  # Extract the numeric part from the transaction ID
        if len(parts) > 2:
            item = parts[2].strip('()')
            return op, tid, item
        return op, tid
